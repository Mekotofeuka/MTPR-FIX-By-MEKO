root@swznrnicae:~# grep -r 'mtpr_syn_fix' /etc/ufw/ --include='*.rules'
/etc/ufw/before.rules:# MTProxy SYN FIX by MEKO (mtpr_syn_fix)
/etc/ufw/before.rules:# MTProxy SYN FIX by MEKO (mtpr_syn_fix)
root@swznrnicae:~#

окак. а вот и баг нашёлся.


вот текущий код, дай мне его обратно и исправляй чтобы дубликаты и тд не создавались и всё удалялось корректно не оставляя от себя ничего:
